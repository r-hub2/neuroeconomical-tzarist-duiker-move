setGeneric("hrBootstrap", function(x, rep = 100, plot = TRUE, level = 95, levelMax = 100, unin = "km", unout = "m2", ...) {
  standardGeneric("hrBootstrap")
})
setMethod("hrBootstrap",
  ...
  definition = function(x, ...) {
    ...
    crds <- coordinates(x)
    dimnames(crds) <- NULL
    locs <- lapply(j, function(js, coords) {
      replicate(n = rep, SpatialPoints(coords[sample(1:nlocs, size = js, replace = T), ]))
    }, coords = crds)
